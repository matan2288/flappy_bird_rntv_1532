...
description: Provides React Native performance optimization guidelines for FPS, TTI, bundle size, memory leaks, re-renders, and animations. Use when debugging jank, frame drops, slow startup, large bundles, memory leaks, FlashList, Hermes, native modules, or React Native performance reviews.
---
...
Before applying optimizations, confirm:
- **Expo CLI** or **React Native CLI** is installed (`npx expo --version`, `npx react-native --version`)
- Metro bundler is running when doing bundle analysis
- React Native DevTools profiling is available via `agent-device react-devtools` when debugging renders (`agent-device react-devtools status`, then `wait --connected`)
